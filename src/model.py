...
ATS_FILE_PATH = os.path.join(BASE_DIR, "..", "data", "final_cleaned.csv")
EMBEDDING_MODEL = "all-MiniLM-L6-v2"
INDEX_FILE = os.path.join(BASE_DIR, "..", "data", "job_embeddings.faiss")
...
class ATSModel:
    ...
    def match_resume(self, resume_data):
        ...
        for score, idx in zip(scores[0], indices[0]):
            if idx >= 0:
                ...
                results.append({
                    "name": job["name"],
                    "score": float(round(score * 100 + 10 ,2)), # large dataset size
                    "details": {
    ...
    def match_custom_jd(self, resume_data, job_description):
        ...
        return float(round(similarity * 100 + 30, 2))
